...
function fitWidth(WrappedComponent) {
	...
	// console.log(withRef,minWidth,arguments.length,arguments[1],arguments[2])
	...
	console.log('resize computed',_window$getComputedSt.height,width,paddingLeft,paddingRight,window.innerHeight,window.innerWidth);
					...
					_this2.setState({
						...
						height: Math.round(Math.max((window.innerHeight-200), 100))
					});
				});
			}
		}, {
	...
	// console.log('render',withRef,this.state.width);
	...
	ResponsiveComponent.displayName = "fitChart(" + getDisplayName(WrappedComponent) + ")";

	return ResponsiveComponent;
}
//# sourceMappingURL=fitChart.js.map